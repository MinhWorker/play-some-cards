...
import type { ClientToServerEvents, JoinedRoom, PlayerId, ServerToClientEvents } from '@psc/shared';
 ...
 * player (filtered per player so hidden cards stay hidden).
 */
...
export class RoomsGateway implements OnGatewayInit, OnGatewayDisconnect {
  ...
  join(socket: AppSocket, req: { roomCode: string; name: string }) {
    return this.handle(() => {
      const { room, player } = this.rooms.join(req.roomCode, req.name);
      return this.enter(socket, room, player);
    });
  }
  ...
  leave(socket: AppSocket) {
    return this.handle(() => {
      ...
      const room = this.rooms.leave(roomCode, playerId);
  ...
  /** Sends each socket in the room its own filtered snapshot. */
