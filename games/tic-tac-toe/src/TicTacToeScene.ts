...
import { type TicTacToeMove, type TicTacToeState, winningLine } from './rules.js';
...
export class TicTacToeScene extends BoardScene<TicTacToeState, TicTacToeMove> {
  ...
  protected build() {
    ...
    this.score = {
      icons: SEATS.map((s) => this.image(0, 0, s.piece)),
  ...
  protected draw() {
    ...
    this.tiles.forEach((tile, cell) => {
      ...
      if (mark && !piece) {
        ...
        piece = this.image(x, y, mark === 'X' ? 'piece-x' : 'piece-o').setScale(0);
        this.tweens.add({
          targets: piece,
