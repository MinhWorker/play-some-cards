...
| `npm run dev` | Server on :3000, web on :5173 (Vite proxies `/socket.io` and `/api` to the server) |
...
| `npm run smoke [url]` | Two bots play a full tic-tac-toe game against a running server (default `http://localhost:3000`) |
